...
public class SurfaceCameraScheme : ICameraMovementScheme
{
    ...
    {
        Vec2Double camSurfaceVector = _localPosition + _tracking.Position - _surface.Position;
        ...
        Vec2Double newCamSurfacePosition;
        if (camSurfaceVector == Vec2Double.Zero) newCamSurfacePosition = Vec2Double.Zero;
        else newCamSurfacePosition = Vec2Double.FromPolar(
    ...
    {
        // handle case where the camera is at the same position as the surface object
        Vec2Double camSurfaceVector = _surface.Position - _tracking.Position - _localPosition; 
        if (camSurfaceVector == Vec2<SDecimal>.Zero) return;
        _localPosition += Vec2Double.FromPolar(camSurfaceVector.Direction(), -(double)distance);
    }
