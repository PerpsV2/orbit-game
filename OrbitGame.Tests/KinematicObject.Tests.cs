...
public class KinematicObject_Tests
{
    ...
    private readonly Vec2<SDecimal> _testVector = new(5, 5);
    ...
    public KinematicObject_Tests()
    {
        ...
        _testKinematicObject2 = _testKinObjTemplate.CreateTestInstance("Object 2", new(
            position: new Vec2<SDecimal>(0, -5),
            angle: -Math.PI / 2
        ));
    }
    ...
    {
        Assert.Equal(new Vec2<SDecimal>(0, 5), _testKinematicObject1.ObjectToWorldSpace(_testVector));
    }

    [Fact]
    public void KinematicObject_WorldToObjectSpaceMethod()
    {
        Assert.Equal(new Vec2<SDecimal>(5, 0), _testKinematicObject1.WorldToObjectSpace(_testVector));
    }

    [Fact]
    public void KinematicObject_ObjectToObjectSpaceMethod()
    {
        Assert.Equal(new Vec2<SDecimal>(-10, 0), _testKinematicObject1.ObjectToObjectSpace(_testVector, _testKinematicObject2));
    }

    [Fact]
    public void KinematicObject_Properties()
    {
        _testKinematicObject1.Acceleration = new Vec2<SDecimal>(5, 0);
        Assert.Equal(new Vec2<SDecimal>(5, 0), _testKinematicObject1.Acceleration);
        ...
        Assert.Equal(new Vec2<SDecimal>(0, 1), _testKinematicObject1.ForwardVector);
        Assert.Equal(new Vec2<SDecimal>(0, -1), _testKinematicObject2.ForwardVector);
        
        Assert.Equal(new Vec2<SDecimal>(1, 0), _testKinematicObject1.RightVector);
        Assert.Equal(new Vec2<SDecimal>(-1, -0), _testKinematicObject2.RightVector);
    }
}