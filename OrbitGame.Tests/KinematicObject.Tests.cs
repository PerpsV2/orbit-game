...
public class KinematicObject_Tests
{
    ...
    {
        Assert.Equal(new DVector2<SDecimal>(0, 5), _testKinematicObject1.ObjectToWorldSpace(_testVector));
    }

    [Fact]
    public void KinematicObject_WorldToObjectSpaceMethod()
    {
        Assert.Equal(new DVector2<SDecimal>(5, 0), _testKinematicObject1.WorldToObjectSpace(_testVector));
    }

    [Fact]
    public void KinematicObject_ObjectToObjectSpaceMethod()
    {
        Assert.Equal(new DVector2<SDecimal>(-10, 0), _testKinematicObject1.ObjectToObjectSpace(_testVector, _testKinematicObject2));
    }

    [Fact]
    public void KinematicObject_Properties()
    {
        _testKinematicObject1.Acceleration = new DVector2<SDecimal>(5, 0);
        Assert.Equal(new DVector2<SDecimal>(5, 0), _testKinematicObject1.Acceleration);
        ...
        Assert.Equal(new DVector2<SDecimal>(0, 1), _testKinematicObject1.ForwardVector);
        Assert.Equal(new DVector2<SDecimal>(0, -1), _testKinematicObject2.ForwardVector);
        
        Assert.Equal(new DVector2<SDecimal>(1, 0), _testKinematicObject1.RightVector);
        Assert.Equal(new DVector2<SDecimal>(-1, -0), _testKinematicObject2.RightVector);
    }
}